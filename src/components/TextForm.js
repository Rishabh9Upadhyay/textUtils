...
// using settext(new text) is currect way to change
  ...
  return (
    ...
    <div className='container' style={{color: p.mode==='dark'?'white':'#042743'}}>
      ...
      <button disabled={text.length===0} className="btn btn-primary my-2 mx-2" onClick={handleLwClick}>LowerCase</button>
      <button disabled={text.length===0} className="btn btn-primary mx-2" onClick={SpeekContents}>Listen Contents</button>
      <button disabled={text.length===0} className="btn btn-primary mx-2" onClick={clearText}>Clear Contents</button>
      <button disabled={text.length===0} className="btn btn-primary my-2 mx-2" onClick={SpaceRemove}>Space Remove</button>
    ...
    <div className="container my-3" style={{color: p.mode==='dark'?'white':'#042743'}}>
        ...
        <li>Number of words {text.split(/\s+/).filter((element)=>{return element.length!==0;}).length}</li> 
        <li>{0.008*(text.split(/\s+/).filter((element)=>{return element.length!==0;}).length)} minuts to read</li>
        <li><b>Preview: </b></li>
...
// text.split(" ") also consider space as list content need to remove extra spaces using filtor